<script setup>
import { digits, colorize } from '../use/ilca'

const qna = {
	"What is ILCA?": "It's a system for using letters for duodecimal counting and unique items accounting.",
	"What letters are used?": "It's the 12 letters that are common in English and Cyrillic alphabets.",
...
<template lang='pug'>
.p-0
	.text-4vw.gap-2vw.text-center.flex.justify-center
		.p-1(
			v-for="digit in digits" :key="digit"
			:style="{color: colorize(digit)}"
			) {{ digit }}
